...
test("并发发送与返回值请求共用一次事件监听初始化", {
	...
	const togglePromise = emitAudioThread("resumeOrPauseAudio");
	...
	await Promise.all([pausePromise, togglePromise, responseAssertion]);
	...
	assert.deepEqual(
		calls
			...
			.sort(),
		["pauseAudio", "resumeAudio", "resumeOrPauseAudio"],
	);
});
