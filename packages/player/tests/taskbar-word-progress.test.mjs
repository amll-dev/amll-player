...
test("预滚动进入下一句时，逐字高亮仍等待真实演唱时间", async () => {
	const { findDisplayedLyricIndex } = await import("../src/pages/taskbar-lyric/lyric-timeline.ts");
	const word = { word: "下一句", startTime: 2_000, endTime: 3_000 };
	const lines = [{ startTime: 500, endTime: 1000, words: [] }, { startTime: 2000, endTime: 3000, words: [word] }];
	assert.equal(findDisplayedLyricIndex(lines, 1500), 1);
	assert.equal(getTimedWordProgress(word, 1500), 0);
	assert.equal(getTimedWordProgress(word, 1999), 0);
	assert.equal(getTimedWordProgress(word, 2500), 0.5);
});
